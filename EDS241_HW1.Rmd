...
```{r}
model_1 <- estimatr::lm_robust(pm2_5 ~ low_birth_weight, data = data_sheet1)
model_1
```
...
```{r}
pm2_5_reduced <- dplyr::mutate(data_sheet1, pm2_5 = pm2_5 - 2) %>% 
  modelr::add_predictions(model = model_1)
```

## f) Add the variable Poverty as an explanatory variable to the regressionin (d). Interpret the estimated coefficient on Poverty. What happens to the estimated coefficient on PM25, compared to the regression in (d). Explain.
```{r}
model_2 <- estimatr::lm_robust(pm2_5 ~ low_birth_weight + poverty, data = data_sheet1)
model_2
```
The estimated coefficient of poverty in this model is 0.026. This can be interpreted to mean that for every 1% increase in "poverty" there is an expected 0.026 increase in PM2.5 for the census tract while holding low birth weight percentage constant. The estimated coefficient for PM2.5 decreases from 9.13 to 8.81 in this model compared to the previous. This happens because this coefficient now predicts the PM2.5 level for a census tract with no low birth rates and no poverty. This is an unlikely scenario but is the models prediction based on the regression.
